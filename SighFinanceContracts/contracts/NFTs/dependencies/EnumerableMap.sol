...
library EnumerableMap {
     ...
     */
    function _set(Map storage map, bytes32 key, boosterInfo _NFTvalue) private returns (bool) {        
    ...
    */
     ...
     * Returns true if the key was added to the map, that is if it was not already present.
     */
    function set(UintToNFTMap storage map, uint256 key, boosterInfo _NFT_value) internal returns (bool) {
    ...
    */
    function at(UintToNFTMap storage map, uint256 index) internal view returns (uint256, address, string memory) {
        (bytes32 key, address _owner, string _type) = _at(map._inner, index);
