...
library EnumerableMap {
    ...
    // Tupple contains the owner and the type information of a SIGH Finance's Booster NFT
    struct tupple {
        address owner;
        string _type;
    }

    // key is the tokenID and _NFT_Value is the tupple struct
    struct MapEntry {
        bytes32 _key;
        tupple _NFT_Value;
    }
    ...
    struct Map {
        MapEntry[] _NFTs;                       // Storage of NFT's keys and tupples        
     ...
     */
    function _set(Map storage map, bytes32 key, tupple _NFTvalue) private returns (bool) {
        ...
        if (keyIndex == 0) { // Equivalent to !contains(map, key)
            ...
            map._NFTs[keyIndex - 1]._value = value;
            return false;
        }
    }
    ...
    function _remove(Map storage map, bytes32 key) private returns (bool) {
        ...
        if (keyIndex != 0) { // Equivalent to contains(map, key)
            // To delete a key-value pair from the _NFTs array in O(1), we swap the entry to delete with the last one
    ...
    */
    function _at(Map storage map, uint256 index) private view returns (uint,address,string memory) {
    ...
    function _get(Map storage map, bytes32 key, string memory errorMessage) private view returns (bytes32) {
        ...
        return map._NFTs[keyIndex - 1]._value; // All indexes are 1-based
    }
     ...
     * already present.
     */
    function set(UintToNFTMap storage map, uint256 key, tupple _NFT_value) internal returns (bool) {
    ...
    * - `index` must be strictly less than {length}.
    */
    function at(UintToNFTMap storage map, uint256 index) internal view returns (uint256, tupple) {
        (bytes32 key, tupple _value) = _at(map._inner, index);
        return (uint256(key), _value );
    }
     ...
     * - `key` must be in the map.
     */
    function get(UintToNFTMap storage map, uint256 key) internal view returns (address) {
     ...
     */
    function get(UintToNFTMap storage map, uint256 key, string memory errorMessage) internal view returns (address) {
