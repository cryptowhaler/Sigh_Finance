...
contract AaveIncentivesController is IAaveIncentivesController, VersionedInitializable, AaveDistributionManager {
  ...
  constructor(IERC20 rewardToken,address rewardsVault,IStakedAave psm,uint256 extraPsmReward,address emissionManager,uint128 distributionDuration) public AaveDistributionManager(emissionManager, distributionDuration) {
  ...
  function initialize() external initializer {    
    REWARD_TOKEN.approve(address(PSM), type(uint256).max);      // to unlock possibility to stake on behalf of the user
  }
   ...
   **/
  function claimRewards(address[] calldata assets,uint256 amount,address to,bool stake) external override returns (uint256) {
    if (amount == 0) {
      return 0;
    }
