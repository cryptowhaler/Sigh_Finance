...
abstract contract DebtTokenBase is IncentivizedERC20, VersionedInitializable, ICreditDelegationToken {
  ...
  mapping(address => uint256) internal platformFee;
  mapping(address => uint256) internal reserveFee;

  /**
   * @dev Only lending pool can call functions marked by this modifier
   **/
  modifier onlyLendingPool {
    require(_msgSender() == address(POOL), "Only lending pool can call this function");
    _;
  }

  /**
   * @dev The metadata of the token will be set on the proxy, that the reason of passing "NULL" and 0 as metadata
   */
  constructor( address pool, address underlyingAssetAddress, string memory name, string memory symbol, address incentivesController) public IncentivizedERC20(name, symbol, 18, incentivesController) {
