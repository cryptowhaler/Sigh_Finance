...
contract AToken is VersionedInitializable, IncentivizedERC20, IAToken {
    ...
    modifier onlyLendingPool {
        require(_msgSender() == address(POOL),"Caller must be lending pool contract");
        _;
    }

    constructor(  ILendingPool pool,  address underlyingAssetAddress,  address reserveTreasuryAddress,  string memory tokenName,  string memory tokenSymbol,  address incentivesController ) public IncentivizedERC20(tokenName, tokenSymbol, 18, incentivesController) {
        POOL = pool;
    ...
    function initialize( uint8 underlyingAssetDecimals, string calldata tokenName, string calldata tokenSymbol ) external virtual initializer {
        uint256 chainId;
        ...
        DOMAIN_SEPARATOR = keccak256(  abi.encode(   EIP712_DOMAIN, keccak256(bytes(tokenName)), keccak256(EIP712_REVISION), chainId, address(this))  );
    ...
    */
    function mintToTreasury(uint256 amount, uint256 index) external override onlyLendingPool {
        ...
        // In that case, the treasury will experience a (very small) loss, but it wont cause potentially valid transactions to fail.
    ...
    **/
    ...
    /**
    * @dev Returns the scaled balance of the user. The scaled balance is the sum of all the updated stored balance divided by the reserve's liquidity index at the moment of the update
    * @param user The user whose balance is calculated
    ...
    **/
    function _transfer(address from,address to,uint256 amount,bool validate) internal {
        ...
        if (validate) {
        POOL.finalizeTransfer( UNDERLYING_ASSET_ADDRESS,from,to,amount,fromBalanceBefore,toBalanceBefore);
        }
