    ...
    async borrow() {   //BORROW --> TO BE CHECKED
      ...
      if (!this.$store.state.web3) {
        ...
        if ( !instrumentGlobalStateConfig.borrowingEnabled  ) {
          ...
          if (response.status) {      
            this.$showSuccessMsg({title: "BORROW SUCCESSFUL", message: "BORROW SUCCESS : " + Number(quantity).toFixed(4) + "  " +  this.selectedInstrument.symbol +  " worth " + this.formData.borrowValue + " USD have been successfully borrowed from SIGH Finance. Gas used = " + response.gasUsed });
            ...
            this.$showErrorMsg({title: "BORROW FAILED", message: Number(quantity).toFixed(4) + "  " +  this.selectedInstrument.symbol + " failed to be borrowed from SIGH Finance.  Check Etherescan to undersand why and try again! ", timeout: 7000 }); 
