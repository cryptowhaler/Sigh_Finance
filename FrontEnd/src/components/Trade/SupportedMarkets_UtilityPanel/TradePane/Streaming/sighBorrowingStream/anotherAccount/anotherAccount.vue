...
export default {
  ...
  methods: {
    ...
    ...mapActions(['IToken_redirectBorrowingSIGHStreamOf','getInstrumentPrice','refresh_User_Instrument_State','IToken_getSIGHStreamsAllowances','LendingPoolCore_getUserBasicInstrumentData']),
    ...
    async redirectBorrowingSIGHStreamOf() {                           // RE-DIRECT SIGH STREAM
      ...
      if ( !this.$store.state.web3 || !this.$store.state.isNetworkSupported ) {       // Network Currently Connected To Check
        ...
        let response =  await this.IToken_redirectBorrowingSIGHStreamOf( { iTokenAddress:  this.selectedInstrument.iTokenAddress, _from: this.formData.fromAccount, _to: this.formData.toAccount, symbol : this.selectedInstrument.symbol } );
        if (response.status) {  
    ...
    async  getDetailsOfTheFromAccount() {
        ...
        this.currentAdministratorFromAccount = response.BorrowingSIGHStreamRedirectionAllowance;
        console.log(this.currentAdministratorFromAccount);
